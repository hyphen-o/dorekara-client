  ...
  return (
    <>
      <form css={styles.form} onSubmit={handleSubmit(onSubmit)}>
        ...
        {errors.name?.type === 'required' && <Error>曲のタイトルは必ず入力してください</Error>}
        {errors.name?.type === 'maxLength' && <Error>曲のタイトルは8文字以内で入力して下さい</Error>}
        <div>アーティスト</div>
        <Selector submit={register} text='アーティストを変更' value={song.artist_id}></Selector>
        <div>キー</div>
